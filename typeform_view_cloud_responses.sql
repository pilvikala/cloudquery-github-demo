-- public.typeform_form_cloud_responses source

CREATE OR REPLACE VIEW public.typeform_form_cloud_responses
AS WITH names AS (
         SELECT t1._cq_sync_time,
           ...
           FROM ( SELECT tfr._cq_sync_time,
                  ...
                  WHERE tfr.form_id = 'MZ0DSN82'::text) t1
          WHERE ((((t1.value -> 'field'::text) -> 'id'::text)::json)::text) = '"RJDdgkvvWaie"'::text
        ), emails AS (
         SELECT t1.response_id,
           ...
           FROM ( SELECT tfr.response_id,
                   ...
                   FROM typeform_form_responses tfr
                  WHERE tfr.form_id = 'MZ0DSN82'::text) t1
          WHERE ((((t1.email -> 'field'::text) -> 'id'::text)::json)::text) = '"yX1K7oplSHVX"'::text
        ), answers AS (
         SELECT t1.response_id,
           ...
           FROM ( SELECT tfr.response_id,
                   ...
                   FROM typeform_form_responses tfr
                  WHERE tfr.form_id = 'MZ0DSN82'::text) t1
          WHERE ((((t1.answer -> 'field'::text) -> 'id'::text)::json)::text) = '"HUnsZKqVPDBH"'::text
        )
